...
function FlushOperation(bulkInsert) {	
	this._bulkInsert = bulkInsert;
	this.filename = bulkInsert._generateFilename();
}
...
FlushOperation.prototype.start = function () {
	...
	async.waterfall([
		...
		//this._bulkInsert._deleteFileOnS3(this)
...
FlushOperation.prototype._updateUploadLatency = function() {
	...
	return function(uploadData, callback) {
		self.uploadLatency = Date.now() - self._start;
...
FlushOperation.prototype._updateQueryLatency = function() {
	var self = this;
	return function(queryResults, callback) {
		self.queryLatency = Date.now() - self._start;
	...
	return function (err, results) {
		...
		if (err) {
			if (self.stage = '_executeCopyQuery') {
				self._bulkInsert.emit('flush', err, results, self.copyQuery, self._start, self._bulkInsert, self.stage);
			} else {
				self._bulkInsert.emit('flush', err, null, self.copyQuery, self._start, self._bulkInsert, self.stage);
			}
		}

		self._bulkInsert.emit('flush', null, results, self.copyQuery, self._start, self._bulkInsert, self.stage);	
	};	
};
