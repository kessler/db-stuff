...
FlushOperation.prototype._updateUploadLatency = function() {
	...
	return function(uploadData, callback) {
		self.uploadLatency = Date.now() - self.flushStart;
...
FlushOperation.prototype._updateQueryLatency = function() {
	var self = this;
	return function(queryResults, callback) {
		self.queryLatency = Date.now() - self.flushStart;
...
FlushOperation.prototype.done = function () {
	...
	return function (err, results) {
		...
		// at some point I would like to emit these events only if they were registered or something.

		var bulkInsert = self._bulkInsert;

		// dont leak these outside
		delete self._bulkInsert;		
		delete self._buffer;
