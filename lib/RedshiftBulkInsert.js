...
RedshiftBulkInsert.prototype._getLogFileName = function() {
	return this._tableName + '_' + Date.now() + '_' + process.pid + SUFFIX;
}
...
RedshiftBulkInsert.prototype._onSentCopyToRedshift = function(fileName, start, err, result) {
	...
	var onLogFileRemoved = _.bind(this._decrimentActiveFlushOpsAndEmitFlushEvent, this, start, err, result);
	fs.unlink(pathToFile, onLogFileRemoved);
};

RedshiftBulkInsert.prototype._decrimentActiveFlushOpsAndEmitFlushEvent(start, err, result) {
	this.activeFlushOps--;
