...
function RedshiftBulkInsert(datastore, options, awsOptions) {
	...
	var bucketParts = awsOptions.bucket.split('/');
...
RedshiftBulkInsert.prototype.flush = function () {
	...
	var copyQuery = this._generateCopyQuery(filename);

	var flushOp = this._newFlushOperation(buffer, bufferLength, filename, copyQuery);
...
RedshiftBulkInsert.prototype._newFlushOperation = function (buffer, bufferLength, filename, copyQuery) {
	return new FlushOperation(buffer, bufferLength, filename, copyQuery);
};
...
function FlushOperation(buffer, bufferLength, filename, keyPrefix, copyQuery) {
	EventEmitter.call(this);
...
FlushOperation.prototype.start = function (s3) {
...
FlushOperation.prototype._uploadToS3 = function (s3) {
	var self = this;
	...
	return function(callback) {
		...
		var key = self._keyPrefix + '/' + self.filename;

		s3.putBuffer(buffer, key, function(err, res) {
			if (err) {
				callback(err);
				return;
			}
			if (res.statusCode !== 200) {
				callback('Response status code should be equals to 200');
...
FlushOperation.prototype._executeCopyQuery = function () {
	var self = this;

	return function(data, callback) {
		self.queryStart = Date.now();
		var copyQuery = self._generateCopyQuery(self.filename);
		self.stage = '_executeCopyQuery';
		self.copyQuery = copyQuery;
